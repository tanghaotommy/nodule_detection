...
do
	#Train the detector
	cmd="python main.py --model $detector --save-dir $i --train-filename ./split/train_${i}.npy --val-filename ./split/val_${i}.npy --lr $lr --workers 16 -b $batch_size --epochs $epochs --save-freq 5"
	echo "$cmd"
	...
	do
		cmd="python main.py --model $detector --resume results/${i}/${j}.ckpt --workers 1 --test-filename ./split/val_${i}.npy --test 1 --save-dir /home/htang6/workspace/detector/val/${i}/${j}"
		echo "$cmd"
	...
	cmd="python main.py --model $detector --resume results/${i}/${best_epoch}.ckpt --workers 1 --test-filename ./filenames_test2.npy --test 1 --save-dir test/${i} --n_test 4"
	echo "$cmd"
        $cmd
	errono=$?
	if [ $errono -ne 0 ]
	...
	cmd="python generate_submission.py --save-dir ${submission}/${i}.csv --filename ./split/test_${i}.npy --pred-dir pred/"
	echo "$cmd"
#	$cmd
	errono=$?
	if [ $errono -ne 0 ]
