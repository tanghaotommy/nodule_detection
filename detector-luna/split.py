...
def main():
    ...
    for i in xrange(k_fold):
        ...
        filenames_train = train_subset[len(train_subset) / k_fold:]
        filenames_val = train_subset[:len(train_subset) / k_fold]
        filenames_test = test_subset
        
        print "---------------%d th folder ----------------------" % (i)
        print "Number of training patients: ", len(filenames_train)
        print "Number of validation patients: ", len(filenames_val)
	print "Number of test patients: ", len(filenames_test)
	...
	np.save(os.path.join(save_dir, "test_%d" % (i)), filenames_test)
