...
parser.add_argument('--save-freq', default='10', type=int, metavar='S',
                    help='save frequency')
parser.add_argument('--data', default=None, type=str, metavar='PATH',
...
parser.add_argument('--test-set', default='test', type=str, metavar='SET',
                    help="if testing, use {'test', 'val', 'train'} set")
...
parser.add_argument('--train-filename', default='', type=str, metavar='TRAIN',
                    help='directory to save checkpoint (default: none)')
parser.add_argument('--val-filename', default='', type=str, metavar='VAL',
                    help='directory to save checkpoint (default: none)')
parser.add_argument('--test-filename', default='', type=str, metavar='TEST',
                    help='directory to save checkpoint (default: none)')
...
def train(data_loader, net, loss, epoch, optimizer, get_lr, save_freq, save_dir):
    ...
    metrics = []
    for i, (data, data2, target, coord) in enumerate(data_loader):
        data = Variable(data.cuda(async = True))
        data2 = Variable(data2.cuda(async = True))
        target = Variable(target.cuda(async = True))
        coord = Variable(coord.cuda(async = True))
        print data.size()
        print data2.size()
        output = net(data, data2, coord)
...
def validate(data_loader, net, loss):
    ...
    for i, (data, data2, target, coord) in enumerate(data_loader):
        ...
        data2 = Variable(data2.cuda(async = True), volatile = True)
...
def test(data_loader, net, get_pbb, save_dir, config, test_set):
    start_time = time.time()
    # eg test2_100
    save_dir = os.path.join(save_dir,'bbox/',test_set + '_' + os.path.splitext(os.path.basename(args.resume))[0])
    ...
    for i_name, (data, data2, target, coord, nzhw) in enumerate(data_loader):
        s = time.time()
        ...
        for i in range(len(splitlist)-1):
            ...
            if isfeat:
                output,feature = net(input, data2, inputcoord)
	...
	print 'number of pbbs: ', len(pbb)
        pbb = nms(pbb, 0.1)
	print 'number of pbbs finally: ', len(pbb)
